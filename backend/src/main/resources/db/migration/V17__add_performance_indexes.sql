...
-- MySQL 8.0.19+ and H2 compatible: uses CREATE INDEX IF NOT EXISTS
...
CREATE INDEX IF NOT EXISTS idx_applications_candidate_status ON applications(candidate_id, status, applied_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_saved_jobs_candidate_date ON saved_jobs(candidate_id, created_at DESC);

-- Saved jobs: candidate + job for existence checks
CREATE INDEX IF NOT EXISTS idx_saved_jobs_candidate_job ON saved_jobs(candidate_id, job_id);

-- Job alerts: active + candidate for alert matching
CREATE INDEX IF NOT EXISTS idx_job_alerts_active_candidate ON job_alerts(active, candidate_id);
