...
export default function RegisterCandidatePage() {
  ...
  const [form, setForm] = useState<Form>({ fullName: '', email: '', password: '', confirmPassword: '' });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await authService.registerCandidate(form);
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-body-md text-neutral-500">
            Join as a candidate to find your next opportunity
          </p>
        </div>
